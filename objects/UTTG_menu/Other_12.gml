...
if (_menu == 0) {
	if(_mode==0){
		...
		_inst_settings.override_color_text=(_choice==1 ? c_yellow : c_white);
	}
}
	...
	var i = 0;
	while (i < 4) {
		_inst_savebox[i]._color=(_choise_save==i ? c_yellow : c_white);
		i++;
	}
}
if (_menu == 4) {
	_inst_savebox[_choise_save]._choice_edit = _choise_save_edit;
}