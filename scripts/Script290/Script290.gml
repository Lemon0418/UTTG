function scr_namingscreen() {
	...
	if(whfad == 1) {
	    ...
	    instance_create_depth(0, 0, -999, obj_whitefader);
	    caster_free(-3);
	    alerm = 0;
	    whfad = 2;
	...
	if(whfad == 2) {
	    ...
	    if(alerm > 179) {
	        instance_create_depth(0, 0, -99, obj_persistentfader);
	        /*if(truereset > 0) {
	...
	if(naming == 3) {
	    ...
	    /*if(ini_section_exists("General") && hasname == 1) {
	        ...
	        if(obj_time.j_ch > 0) {
	            js= 1;
	            draw_set_color(c_white);
	            if(selected3 == 2) draw_set_color(c_yellow);
	        ...
	        draw_text(80, 20, string_hash_to_newline(" --- Instruction ---"));
	        draw_text(80, 50, string_hash_to_newline("[Z or ENTER] - Confirm"));
	        draw_text(80, 68, string_hash_to_newline("[X or SHIFT] - Cancel"));
	        draw_text(80, 86, string_hash_to_newline("[C or CTRL] - In-game Menu"));
	        draw_text(80, 104, string_hash_to_newline("[F4] - Fullscreen"));
	        draw_text(80, 122, string_hash_to_newline("[Hold ESC] - Quit"));
	        draw_text(80, 140, string_hash_to_newline("When HP is 0, you lose."));
			draw_text(80, 158, string_hash_to_newline("[ALT and SHIFT] - Battle test room"));
	        draw_set_color(c_white);
	        if(selected3 == 0) draw_set_color(c_yellow);
	        //if(obj_time.j_ch > 0)
			draw_text(116, 166, string_hash_to_newline("Begin  Game"));
	        if(keyboard_check_pressed(vk_down) && selected3 == 0)
	            selected3 = 1;
	        if(keyboard_check_pressed(vk_up) && selected3 == 1)
	            selected3 = 0;
	        //if (obj_time.j_ch > 0) {
	            draw_set_color(c_white);
	            if(selected3 == 1) draw_set_color(c_yellow);
	            draw_text(116, 186, string_hash_to_newline("Settings"));
	        //} else  selected3 = 0;
